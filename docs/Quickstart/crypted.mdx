...
Potresti chiederti perché non usiamo sempre la parola `--clone`. La scelta è pensata per la tua **sicurezza e velocità**:

* **Evitare errori**: Separando i flag, riduciamo il rischio che un utente attivi per sbaglio una cifratura che non voleva (e di cui magari dimentica la password).
* **Rapidità**: Invece di scrivere comandi lunghissimi come `--clone --with-full-encryption`, ti basta un unico flag specifico per dichiarare la tua intenzione.
