...
* ✅ **RISOLTO (12/6/2026)** — il **bot in loop** sul basket (Amazonbot seguiva link relativi che si concatenano, `/basket/packages/...`, a colpi di 302): aggiunto `static/robots.txt` con `Disallow` su `/basket/`, `/access/` e `/su/`. Attivo dal deploy successivo del sito; i bot educati lo recepiscono entro un giorno o due.
...
* ✅ **RISOLTO (12/6/2026)** — **Kutt fantasma**: in `blog.conf` c'era un blocco `<Location /su/>` che faceva da proxy verso un link shortener Kutt su `127.0.0.1:4000`, ma di Kutt sul server non restava nulla (Docker vuoto, nessun servizio): era già stato smantellato in passato. Rimossi il blocco e l'esclusione `ProxyPass /su/ !` dal vhost, e la riga `Disallow: /su/` dal robots.txt.
