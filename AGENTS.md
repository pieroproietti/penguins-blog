...
The source of **https://penguins-eggs.net** — blog, documentation and landing pages for the penguins-eggs and oa-tools projects. Built with **Docusaurus 3.9** (React 19), package manager **pnpm**.

Sibling repos on this machine:
- `~/penguins-eggs` — the TypeScript remaster tool (see its `AGENTS.md` for the full architecture map);
- `~/oa-tools` — the C/Go successor (see `AGENTS.md` and `DOCS/`).
...
`static/llms.txt` is served at `https://penguins-eggs.net/llms.txt` and follows the [llms.txt standard](https://llmstxt.org): it is the entry point AI assistants use to answer questions about penguins-eggs and oa-tools.

**When the oa-tools DOCS tree or CLI changes, this file must be updated too.** Its links point to `oa-tools/DOCS/architecture|design|development|manual/` on GitHub — verify they resolve before committing. Do not let it drift into describing features that do not exist (or denying ones that do).
